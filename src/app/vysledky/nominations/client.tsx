...
import { Check, Lock, Trophy, Film, Users, LayoutGrid } from 'lucide-react';
import type { UserData, CategoryInfo } from './types';
import { ByCategoriesView } from './by-categories';
...
type TabId = 'by-users' | 'by-categories';
...
export function NominationResultsClient({
  ...
  const [activeTab, setActiveTab] = useState<TabId>('by-users');
  ...
  return (
    <div>
      ...
      <div role="tablist" className="tabs tabs-bordered mb-6">
        <button
          role="tab"
          className={`tab gap-2 ${activeTab === 'by-users' ? 'tab-active' : ''}`}
        ...
        <button
          role="tab"
          className={`tab gap-2 ${activeTab === 'by-categories' ? 'tab-active' : ''}`}
          onClick={() => setActiveTab('by-categories')}
        >
...
/* ────────────────────────────────────────── */
/*  Tab 1 – Dle účastníků (original view)    */
/* ────────────────────────────────────────── */

function ByUsersView({
  users,
  categoryMap,
  selectedUserId,
  selectedUser,
